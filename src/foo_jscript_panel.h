...
namespace jsp
{
	...
	static constexpr const char* component_version = "2.4.0-dev";
	...
	static constexpr size_t version = 2400;
...
#define TO_INT(num) (num == SIZE_MAX ? -1 : static_cast<int>(num)) /* Only use on fb2k API methods that return SIZE_MAX / pfc_infinite to indicate failure. */
#define TO_UINT(num) static_cast<uint32_t>(num)

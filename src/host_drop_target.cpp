...
host_drop_target::host_drop_target(panel_window* host) : IDropTargetImpl(host->m_hwnd), m_host(host), m_action(new com_object_impl_t<DropSourceAction>()) {}
...
HRESULT host_drop_target::OnDrop(IDataObject* pDataObj, DWORD grfKeyState, POINTL pt, DWORD* pdwEffect)
{
	...
	if (SUCCEEDED(hr))
	{
		droppedData.to_handles_async_ex(
			...
			fb2k::service_new<helpers::jsp_process_locations_notify>(m_action->m_playlist_idx, m_action->m_base, m_action->m_to_select)
		);
	}

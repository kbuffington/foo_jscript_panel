...
public:
	...
	LRESULT OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		::SendMessage(this->GetParent(), m_fCancel ? WM_USER_PROP_CANCELPROPERTY : WM_USER_PROP_UPDATEPROPERTY, 0, (LPARAM)m_hWnd);
		return lRes;
	}
	...
	LRESULT OnKeyDown(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
	{
		...
		case VK_RETURN:
		{
			...
			if (idx >= 0)
			{
				...
				SetWindowText(text);
				// Announce the new value
				::SendMessage(this->GetParent(), WM_USER_PROP_UPDATEPROPERTY, 0, (LPARAM)m_hWnd);
			}
		}
		::SetFocus(this->GetParent());
		break;
		case VK_ESCAPE:
			...
			::SendMessage(this->GetParent(), WM_USER_PROP_CANCELPROPERTY, 0, (LPARAM)m_hWnd);
			::SetFocus(this->GetParent());
			break;
		}
		bHandled = FALSE;
